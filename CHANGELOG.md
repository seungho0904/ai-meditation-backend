...
- **Demo fixes:** merge default dev CORS origins (fixes `.env` missing `127.0.0.1`); uvicorn `--reload-dir app` only (stops `.venv` reload loops); clearer LLM/TTS/proxy API errors; `npm run dev:api` / improved `smoke` messages.
